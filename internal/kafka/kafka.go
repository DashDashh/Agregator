...
func (s *Service) processOperatorMessage(_ context.Context, msg kafkago.Message) {
	...
	case models.MsgOrderResult:
		...
		if s.store.UpdateOrderStatus(p.OrderID, newStatus) {
			log.Printf("[kafka] order_result applied order_id=%s success=%v new_status=%s",
				p.OrderID, p.Success, newStatus)
		} else {
			log.Printf("[kafka] order_result: order not found order_id=%s", p.OrderID)
		}
